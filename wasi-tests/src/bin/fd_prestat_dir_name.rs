...
unsafe fn test_valid_path(path: String) -> u8 {
    ...
    print_err("failed to find dir".to_string());
...
unsafe fn test_zero_one_two() -> u8 {
    ...
    match fd_prestat_dir_name(0, dst.as_mut_ptr(), 5) {
        ...
        _ => {
            print_err("file descriptor 0 should return NOTDIR Error".to_string());
            return 1;
        }
    }
    ...
    match fd_prestat_dir_name(1, dst.as_mut_ptr(), 6) {
        Err(wasi::ERRNO_NOTDIR) => (),
        _ => {
            print_err("file descriptor 1 should return NOTDIR Error".to_string());
            return 1;
        }
    }
    ...
    match fd_prestat_dir_name(2, dst.as_mut_ptr(), 6) {
        Err(wasi::ERRNO_NOTDIR) => (),
        _ => {
            print_err("file descriptor 2 should return NOTDIR Error".to_string());
            return 1;
        }
    }
...
fn main() -> ExitCode {
    ...
    print_warning("Testing fd_prestat_dir_name ...".to_string());

    unsafe {
        if test_valid_path(arg) == 0 && test_zero_one_two() == 0 {
            print_succ("fd_prestat_dir_name tests passed \u{2713}".to_string());
            return ExitCode::SUCCESS;
        } else {
            print_err("fd_prestat_dir_name tests failed \u{2717}".to_string());
